...
public interface FeedRepository extends JpaRepository<Feed, Integer> {
    ...
    @Query(value ="SELECT * FROM Feed f WHERE (f.FEED_VISIBILITY = 0 or f.HEAD_QUARTER_ID = :headQuarterid) " +
    ...
    @Query("select new br.com.brigadadoslobos.gerenciador.domains.dtos.summarys.FeedSummaryDTO(" +
            "f.id, f.postDate, f.reunionDate, f.title, f.text, h.id, h.description, f.feedVisibility) " +
            "from Feed f " +
    ...
    @Query("select new br.com.brigadadoslobos.gerenciador.domains.dtos.summarys.FeedSummaryDTO(" +
            ...
            "from Feed f " +
            "left join f.headQuarter h " +
            "where (h.id = :headQuarterid or f.feedVisibility = 0) and f.reunionDate between :begin and :end")
    Optional <List<FeedSummaryDTO>> findSummariesByHeadQuarterIdAndPeriod(@Param("headQuarterid") Integer headQuarterid,
